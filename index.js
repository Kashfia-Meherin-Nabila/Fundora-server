  ...
  try {
      ...
      try {
        if (event.type === "invoice.paid") {
          ...
          const creditPackages = {
            ...
            [process.env.STRIPE_PRICE_VALUE]: {
              package_name: "Value",
              credits: 800,
              amount: 60,
            },
          ...
          console.log(
            "Invoice line count:",
            fullInvoice.lines?.data?.length || 0,
          );

          // ==========================================
          // FIND CORRECT PACKAGE LINE
          // ==========================================

          const lineItem = fullInvoice.lines?.data?.find((line) => {
            const priceId =
              line.price?.id || line.pricing?.price_details?.price;

            return priceId && creditPackages[priceId];
          });

          if (!lineItem) {
            console.error("❌ No matching Fundora package found in invoice.");

            console.log(
              "Invoice prices:",
              fullInvoice.lines?.data?.map((line) => ({
                price: line.price?.id,
                pricing: line.pricing?.price_details?.price,
              })),
            );

            return res.status(200).json({
              received: true,
              message: "No matching Fundora package.",
            });
          }
          ...
          const selectedPackage = creditPackages[priceId];
          ...
          const email = metadataEmail?.toLowerCase() || customerEmail;

          console.log("👤 Supporter email:", email);

          if (!email) {
            //console.error("❌ Supporter email not found.");
          ...
          if (!supporter) {
            //console.error("❌ Supporter not found:", email);
          ...
          if (supporter.role !== "Supporter") {
           // console.error("❌ User is not a Supporter:", email);
            ...
            return res.status(200).json({
              received: true,
              message: "Not a supporter.",
            });
          }

          //console.log("👤 Supporter:", supporter.name);
          ...
          if (existingPayment) {
            //console.log("⚠️ Invoice already processed:", invoice.id);
    ...
    app.post("/api/contributions", async (req, res) => {
      try {
        ...
        if (amount < Number(campaign.minimum_contribution)) {
          return res.status(400).json({
            success: false,
        ...
        console.log("Updated supporter:", updatedSupporter);
        ...
        res.status(201).json({
          ...
          remainingCredits: Number(updatedSupporter.credits),

          newRaisedAmount: Number(campaign.raised_amount || 0) + amount,
        });
      } catch (error) {
